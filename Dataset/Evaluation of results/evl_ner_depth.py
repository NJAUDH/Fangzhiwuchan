...
def extract_depth(ner_data):
    extracted_data = defaultdict(list)
    for identity, entities in ner_data.items():
        for entity in entities:
            extracted_data[identity].append((entity['entity'], entity['type'],entity['depth']))
    return extracted_data
...
def main():
    ...
    pred_depth = extract_depth(pred_data)
    true_depth = extract_depth(true_data)


    entity_results = calculate_metrics_1(pred_entities, true_entities, entity_types)
    depth_results = calculate_metrics_1(pred_depth, true_depth, entity_types)
    accuracy = calculate_accuracy(pred_positions, true_positions)

    all_precision,all_recall,all_f1=calculate_metrics_all(pred_entities,true_entities)
    print(f"Position Matching Accuracy: {accuracy:.4f}")
    ...
    for entity_type, metrics in entity_results.items():
        ...
        print()
    print("Depth Results:")
    for entity_type, metrics in depth_results.items():
        print(f"Entity Type: {entity_type}")
        print(f"  Precision: {metrics['precision']:.4f}")
        print(f"  Recall: {metrics['recall']:.4f}")
        print(f"  F1 Score: {metrics['f1']:.4f}")
